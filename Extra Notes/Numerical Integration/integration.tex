...
\begin{figure}[htbp]
\begin{center}
\includegraphics[height=0.65\textheight]{./resources/simpson.png}
\end{center}
\end{figure}
...
\end{itemize}
\alert{Always validate your rule}: integrate $f = 1$, $f = x$, $f = x^2$ first --- you know those answers, and it tells you which kernel your nodes/weights actually assume.
\end{frame}

\begin{frame}{Example: Gauss-Hermite}
...
\item Low-discrepancy sequences (Halton, Sobol, etc.)
\item Modified Latin Hypercube Sampling (MLHS): Hess, Train, Polak (2006) --- a workhorse for mixed logit.
...
\end{itemize}
Modern home in Python: \texttt{scipy.stats.qmc} (Halton, Sobol, LHS); \texttt{pyblp.Integration} ships product rules, sparse grids, Halton, and MLHS behind one interface.
\end{frame}
...
\item Sparse Grids show how to combine 1-D rules more efficiently (next slide)
\end{itemize}
\end{frame}

\begin{frame}{Sparse Grids (Smolyak)}
Idea: instead of the full tensor product, take a clever \alert{combination of small tensor products} of 1-D rules (Smolyak 1963; for the Gaussian kernel: Heiss and Winschel 2008, \url{https://www.sparse-grids.de}).
\begin{itemize}
\item A level-$K$ sparse grid is exact for all polynomials of \alert{total degree} $\leq 2K-1$ --- it gives up the cross terms like $x_1^{8} x_2^{8}$ that the tensor rule pays dearly for.
\item Node counts ($K=5$): $d=4$: \alert{385} vs $5^4 = 625$ tensor; $d = 10$: \alert{8{,}761} vs $5^{10} \approx 9.8$ million.
\item Precomputed nodes/weights for the normal kernel (GQN) are downloadable; or build them yourself from \texttt{hermgauss} via the combination formula.
\end{itemize}
\end{frame}

\begin{frame}{Sparse Grids: Caveats}
Learned the hard way (see PS0):
\begin{itemize}
\item Weights are \alert{negative} for many nodes --- the rule is not a probability distribution, and cancellation can amplify error when the integrand is far from a low-degree polynomial.
\item Designed for functions dominated by \alert{low-order interactions}; a strongly ``diagonal'' integrand over a wide range converges slowly.
\item Always run the monomial checks ($1, x_k, x_k^2, x_1^2x_2^2$) before trusting any downloaded rule.
\end{itemize}
\end{frame}

\begin{frame}{Know Your Integrand (Before Brute Force)}
The binomial mixed logit from PS0:
\begin{align*}
\E[\Lambda(\beta'X)] \quad \text{with} \quad \beta \sim N(\mu, \text{diag}(\sigma^2)), \quad \beta \in \mathbb{R}^4
\end{align*}
\begin{itemize}
\item This is a \alert{ridge function}: it varies only along the direction $X$. And $\beta'X \sim N\left(\mu'X, \sum_k \sigma_k^2 X_k^2\right)$ --- so the ``4-D'' integral is \alert{exactly 1-D}.
\item \texttt{scipy.integrate.nquad} finds the answer with $\sim$10 million evaluations; 1-D \texttt{quad} on the reduced integral needs a few dozen.
\item A tensor rule inherits its 1-D accuracy along the ridge --- which is why it can look unbeatable in low $d$ (and still dies at $d=10$).
\item The multinomial version depends on $J$ indices $\{\beta'x_j\}$, so the reduction is only to $\min(d, J)$ dimensions --- the curse is real in BLP-land, but \alert{effective dimension} is often lower than nominal dimension.
\end{itemize}
\end{frame}
...
\item Add back in the mean $\symbf{Y}=\symbf{\mu} + L \cdot \symbf{Z} \sim N(\symbf{\mu},\Sigma)$
\end{enumerate}
\end{frame}
