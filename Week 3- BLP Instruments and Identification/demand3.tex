...
\item Can we find things that are orthogonal to the error between observed and predicted marketshares?
...
\frametitle{Caveats}
\begin{itemize}
\item We do need a technical condition. This only works if the marketsize $N \rightarrow \infty$.
...
\item Because all constraints hold at the optimium only: less impact of numerical error in tolerance or integration.
...
\begin{frame}{Supply Side / Instruments}
Take FOC of Bertrand-Nash Pricing:\\
\begin{eqnarray*}
s_j(\mathbf{p}) + \sum_{k \in \mathcal{J}_f} (p_k - c_k) \frac{\partial s_k(\mathbf{p_t})}{\partial p_j} &=& 0
\end{eqnarray*}
\begin{eqnarray*}
