...
\begin{itemize}
\item We can use the same $\hat{e}_{jt}$ approach and just do MLE like McFadden and Train instead.
...
\begin{frame}{MDPLE estimator}
    ...
    \begin{itemize}
        ...
        \begin{enumerate}
            \item the \textbf{MDLE}
                \begin{align}
                    \log\hat{L}(\theta,\delta) =\sum_{m=1}^M\sum_{j=0}^{J_m}\sum_{i=1}^{N_m} d_{ijm}(D_{im}\log \sigma_{jm}^{y_{im}}(\theta,\delta) + (1-D_{im})\log\sigma_{jm}(\theta,\delta))\\
                     =\sum_{m=1}^M\sum_{j=0}^{J_m}\sum_{i=1}^{N_m} D_{im} d_{ijm} \log\frac{\sigma_{jm}^{y_{im}}}{\sigma_{jm}} + \sum_{m=1}^M N_m\sum_{j=0}^{J_m}s_{jm}\log\sigma_{jm}
                \end{align}
...
\begin{enumerate}
    ...
    \item GMM version of the estimator (= MDLE replaced with score moments): asymptotically equivalent, but the GMM can suffer from the multiplicity of optima (while the MDPLE is convex in $\delta$)
    \item imposing share constraints (BLP estimators): equivalent with MDPLE with infinite weights on some moments (loss of efficiency)
...
\begin{frame}{Micro BLP is used a lot}
    \begin{wideitemize}
        ...
        \end{enumerate}
        \item Dozens of applications: automobiles, insurance, retail, schools, distilled spirits, \ldots
        \begin{itemize}
            \item See the table of examples (and their micro moments) in a few slides
        \end{itemize}
    \end{wideitemize}
\end{frame}
...
\begin{frame}[label=framework]{A standardized framework (Conlon Gortmaker 2023)}
    \begin{wideitemize}
        ...
        \begin{itemize}
            ...
            \item \alert{Market shares} $\sigma_{jt} = \sum_i w_{it} \sigma_{ijt}$ integrate over consumer mass, each type has known weight $w_{it}$
        \end{itemize}
...
\begin{frame}[label=standard]{Standard micro moments}
    ...
    \begin{wideitemize}
        ...
        \begin{itemize}
            \item Only \alert{indirectly}: $\beta_0$ enters $\sigma_{ijt}$ only through $\delta_{jt} = x_{jt}'\beta_0 + \xi_{jt}$, pinned down by share constraint
        \end{itemize}
    \end{wideitemize}
\end{frame}
...
\begin{frame}[label=estimator]{Micro BLP estimator}
    ...
    \begin{wideitemize}
        ...
        \begin{equation*}
            \overline{v}_p = \frac{1}{N_{d_p}} \sum_n v_{pi_nj_nt_n} \xrightarrow{P_\mathrm{A}} v_p(\theta_0) = \E_\mathrm{A}[v_{pi_nj_nt_n}] = \frac{\sum_t \sum_i \sum_j w_{it} \sigma_{ijt}(\theta_0) w_{d_pijt} v_{pijt}}{\sum_t \sum_i \sum_j w_{it} \sigma_{ijt}(\theta_0) w_{d_pijt}}
        \end{equation*}
    \end{wideitemize}
\end{frame}
