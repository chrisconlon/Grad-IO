...
%    & s_j = \sum_{i} \pi_i s_{i,j} \text{ for all } j \in \mathcal{J}\\
    & \symbf{D}= \frac{1}{\symbf{s}}\sum_{i = 1}^I w_i \, \symbf{\sigma_i} \times \left[\frac{\symbf{\sigma_{i}}}{1-\symbf{\sigma_i}}\right]^T
\end{align*}
...
\begin{column}{0.5\textwidth}
    \begin{itemize}
        \item Each column of the matrix $\symbf{S}$ ($I \times J$) represents a ``type'' $\symbf{\sigma_i}$ (a $J \times 1$) vector of choice probabilities.
        \item We see both the \alert{sparsity} as well as specific consumer \alert{segments} (or ``types'').
        \item We also obtain the share of each type in the population $w_i$.
        \item Remember that $u_{ij} - u_{i0} = \log \sigma_{ij} - \log \sigma_{i0}$, so that we identify indirect utilities (relative to outside good).
