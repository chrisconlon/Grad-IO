...
\begin{itemize}
\item Gopinath, Itskhoki, Rigobon (2010): 25\% if priced in dollars; 95\% if not.
...
\begin{frame}{Recall: Multiproduct Bertrand and Diversion}
\footnotesize
Recall the Bertrand FOC's for multi-product firm $f$ from the diversion lecture:
\begin{align*}
...
\end{align*}
Multi-product pricing \alert{raises the opportunity cost} of selling $j$.
\end{frame}

\begin{frame}{Recall: Upward Pricing Pressure}
Agencies often calculate \alert{Upward Pricing Pressure}: UPP asks how the merger \alert{changes} the opportunity cost (recall the UPP frames from the diversion lecture):
\begin{align*}
...
\symbf{p^w}  &= \symbf{mc^w} + \left(\mathcal{H}_{w} \odot \left( \frac{\partial \symbf{p^r}}{\partial \symbf{p^w}} \cdot  \Delta_r(\symbf{p^r} ) \right) \right)^{-1} \symbf{\sigma}(\symbf{p^r})
\end{align*}
\begin{itemize}
...
\begin{align*}
\symbf{p^r} &=\symbf{mc^r} + e_k +(\mathcal{H}_r \odot \Delta_{r}(\symbf{p^r}))^{-1} \symbf{\sigma}(\symbf{p^r})\\
\end{align*}
...
\begin{align}
\tag{retailer FOC}
 f(\symbf{p^r},\symbf{mc^r}) &\equiv \symbf{p^r}  - \symbf{mc^r}-  \left(\mathcal{H}_{r} \odot \Delta_r(\symbf{p^r}) \right)^{-1} \symbf{\sigma}(\symbf{p^r})=0
\end{align}
...
Use the substitution $\Omega(\symbf{p^r}) \equiv \mathcal{H}_r \odot \Delta_{r}(\symbf{p^r})$, and differentiate the retailers' system of FOC's with respect to $p_l$, to get the $J \times J$ matrix with columns $l$ given by:
\begin{align}
\frac{\partial f(\symbf{p^r},\symbf{p^w})}{\partial p_l^r} \equiv e_l + \Omega^{-1}(\symbf{p^r})
\left[  \mathcal{H}_{r} \odot \frac{\partial\, \Delta_r(\symbf{p^r})}{\partial\, p_l^r} \right]
\Omega^{-1}(\symbf{p^r})\,
\symbf{\sigma}(\symbf{p^r}) -\Omega^{-1}(\symbf{p^r})\, \frac{\partial \symbf{\sigma}(\symbf{p^r})}{\partial p_l^r}.
\end{align}
The complicated piece is the demand Hessian: a $J \times J \times J$ tensor with elements $(j,k,l)$, $\frac{\partial^2 \sigma_j}{\partial p_k^r \partial p_l^r}$, since $\frac{\partial\, \Delta_r(\symbf{p^r})}{\partial\, p_l^r} = -\frac{\partial^2 \symbf{\sigma}}{\partial \symbf{p^r} \partial p_l^r}$.\\
...
\item Example: a constant sales tax rate $\mathcal{P} \equiv \frac{\partial \symbf{p^r}}{\partial \symbf{p^w}} = \text{diag}(1+\tau_r)$
\end{itemize}
\begin{align*}
\symbf{p^w}  &= \symbf{mc^w} + \left(\mathcal{H}_{w} \odot \left( \frac{\partial \symbf{p^r}}{\partial \symbf{p^w}} \cdot  \Delta_r(\symbf{p^r} ) \right) \right)^{-1} \symbf{\sigma}(\symbf{p^r})
\end{align*}
Adapt the Morrow Skerlos $\zeta$ fixed point where $\mathcal{P} \Delta(\symbf{p}_t) = \mathcal{P} \Lambda_t\left(\symbf{p}_t\right)- \mathcal{P} \Gamma_t\left(\symbf{p}_t\right)$
\begin{align*}
\symbf{p}_t &\leftrightarrow \symbf{c}_t+\symbf{\zeta}_t\left(\symbf{p}_t\right) \quad \text { where }\\
 \symbf{\zeta}_t\left(\symbf{p}_t\right)&=\Lambda_t\left(\symbf{p}_t\right)^{-1} \alert{\mathcal{P}^{-1}}\left[\mathcal{H}_t^* \odot \alert{\mathcal{P}}\, \Gamma_t\left(\symbf{p}_t\right)\right]\left(\symbf{p}_t-\symbf{c}_t\right)-\Lambda_t\left(\symbf{p}_t\right)^{-1} \alert{\mathcal{P}^{-1}} \symbf{\sigma}_t\left(\symbf{p}_t\right)
\end{align*}
For diagonal $\mathcal{P}$ (not sure about general case with Hadamard product):
\begin{align*}
 \symbf{\zeta}_t\left(\symbf{p}_t\right)=\Lambda_t\left(\symbf{p}_t\right)^{-1}\left[\mathcal{H}_t^* \odot  \Gamma_t\left(\symbf{p}_t\right)\right]\left(\symbf{p}_t-\symbf{c}_t\right)-\Lambda_t\left(\symbf{p}_t\right)^{-1} \alert{\mathcal{P}^{-1}} \symbf{\sigma}_t\left(\symbf{p}_t\right)
\end{align*}
\end{frame}
...
\begin{itemize}
\item When PT is high, we think that downstream firms have significant market power
...
\item A huge class of models adds an extra parameter between wholesale and retail prices...
\end{itemize}
\end{frame}
