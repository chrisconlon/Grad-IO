...
\begin{align*}
\delta_{jt}(\symbf{s}_t,\alert{\mathrm{y}_t}, \widetilde{\theta}_2) + \alpha p_{jt} &= h_d(\textrm{x}_{jt}, \, \alert{\textrm{v}_{jt}},\theta_1)  + \xi_{jt} \\
...
\begin{align*}
\delta_{jt}(\symbf{s}_t,\widetilde{\theta}_2) + \alpha p_{jt} &= h_d(\textrm{x}_{jt}, \, \alert{\textrm{v}_{jt}},\theta_1)  + \xi_{jt} \\
...
\item Supply side/testing: exactly the machinery from the previous section -- compute $\eta_{jt}(\symbf{p},\symbf{s},\theta_2,\kappa)$ under each conduct model and compare scalar-moment violations via the RV test.
\end{itemize}
\end{frame}
...
\begin{align*}
\sigma_{j}^{-1}(\symbf{s}_t, \symbf{p_t},\alert{\mathrm{y_t}}, \mathrm{x}_t,  \mathrm{v}_t, \widetilde{\theta}_2)
...
\begin{center}
\scalebox{0.5}{\input{resources/10_20noeprice_final.tex}}
\end{center}
\end{frame}
