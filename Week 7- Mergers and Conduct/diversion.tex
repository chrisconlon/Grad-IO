...
= \frac{Q_k}{\left(\sum_{i \in \mathcal{C}_j} T_i \right)
 -Q_j} \approx \frac{\sigma_k}{1-\sigma_j} \mid \text{ ever chooses } j
\end{align*}
\begin{itemize}
...
C_{j \rightarrow k}
=\frac{\sum_{i \in \mathcal{C}_j} q_{i,k}}{\sum_{i \in \mathcal{C}_j} q_{i,(-j)}} 
%&= \frac{\sum_{i \in \mathcal{C}_j} \sum_{t} 1\left(c_{i t}=k\right)}{\sum_{i \in \mathcal{C}_j} \sum_{t} 1\left(c_{i t} \neq j\right)} 
= \frac{\sum_{i \in \mathcal{C}_j}   T_i \cdot \hat{s}_{ik} }{\sum_{i \in \mathcal{C}_j}  T_i \cdot (1-\hat{s}_{ij}) } 
= \sum_{i \in \mathcal{C}_i} \underbrace{\frac{\hat{s}_{ik}}{1-\hat{s}_{ij}}}_{\hat{D}_{jk,i}} \cdot \underbrace{\frac{q_{i,(-j)}}{\sum_{i' \in \mathcal{C}_j} q_{i',(-j)}}}_{=\gamma_{ij}}.
\end{align*}
\begin{itemize}
...
\begin{enumerate}
 \item the individual outside good choice probability $\sigma_{i0}(\calJ,x) = \frac{1}{ \sum_{k \in \calJ} \exp[V_{ik}]}$;
 \item  that the outside good choice probability after removing $j$ increases by the individual share of $j$ times the individual diversion ratio from $j$ to the outside good $\sigma_{i0}(\calJ\setminus j,x)  = \sigma_{i0}(\calJ,x) + D_{j0,i}(x) \cdot \sigma_{ij}(x)$;
 \item for members of the logit family: $D_{j0,i}(x) = \frac{\sigma_{i0}(x)}{1-\sigma_{ij}(x)}$.
\end{enumerate}
\end{frame}
...
\begin{align*}
WTP_i(j) &=\log \left( \frac{\sigma_{i0}(\calJ\setminus j,x) }{\sigma_{i0}(\calJ,x) }\right)
=\log \left( 1+\frac{D_{j0,i}(x) \sigma_{ij}(x)}{\sigma_{i0}(\calJ,x) }\right)\\
&=\log \left( 1+\frac{\sigma_{i0}(\calJ,x) \cdot \sigma_{ij}(x)}{(1-\sigma_{ij}(x)) \cdot \sigma_{i0}(\calJ,x) }\right)\\
&=\log \left( 1+\frac{\sigma_{ij}(x)}{1-\sigma_{ij}(x)  }\right)
\approx \frac{\sigma_{ij}(x)}{1-\sigma_{ij}(x) }
\end{align*}
