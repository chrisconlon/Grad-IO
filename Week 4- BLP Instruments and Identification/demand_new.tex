...
\small
Recall the multi-product Bertrand FOCs from Week 2, where firms solve $\arg \max_{p \in \calJ_f} \pi_f (\symbf{p}) = \sum_{j \in \calJ_f} (p_j - c_j) \cdot q_j(\symbf{p})$, which rearrange to:
\begin{align*}
...
\begin{frame}
\frametitle{Starting Point: From Individual Choice to Market Shares}
\small
Last lecture: individual choices $(d_{i1},\ldots,d_{iJ},d_{i0}) \sim \text{Categorical} (\sigma_{i1},\ldots,\sigma_{iJ},\sigma_{i0})$ with mixed logit choice probabilities:
\begin{align*}
\sigma_{ij}(\theta)= \Pr(d_{ij}=1) = \int \frac{\exp[\beta_{\iota} \, x_j]}{1+\sum_{k \in \calJ} \exp[\beta_{\iota} \, x_k]}\, f(\beta_{\iota} \mid \theta)\, \partial \beta_{\iota}
\end{align*}
If each individual is \alert{exchangable} then ex-ante they have the same choice probabilities: $\sigma_{ij}=\sigma_j$, and the sum of $M$ Categoricals is Multinomial:
\begin{align*}
(q_{1}^{*},\ldots,q_{J}^{*},q_{0}^{*}) \sim \text{Mult} (M, \sigma_{1},\ldots,\sigma_{J},\sigma_{0} )
\end{align*}
where $q_{j}^{*}=\sum_{i=1}^M d_{ij}$ is a \alert{sufficient statistic}.
\begin{itemize}
\item If $M$ gets large enough then $(\frac{q_1}{M},\ldots,\frac{q_J}{M},\frac{q_0}{M})\rightarrow (s_1,\ldots,s_J,s_0)$
 \item Idea: Equate observed market shares to the conditional choice probabilities $(\sigma_1(\symbf{x_i},\theta),\ldots,\sigma_J(\symbf{x_i},\theta),\sigma_0(\symbf{x_i},\theta))$.
\item  Challenges: We probably don't really observe $q_0$ and hence $M$.
\item Introduce idea of market $t$ (otherwise not much data!)
\item MLE on individual data (and why MSLE is \alert{inconsistent} under simulation error) was covered in the mixed logit lecture.
\end{itemize}
\end{frame}
...
\vspace{-0.5cm}
\begin{align*}
\nonumber \min_{w_i} \sum_{j,t} \left(s_{jt} - \sum_i w_i \cdot \sigma^{*}_{ijt}(\beta^{*}_i, \symbf{x_t})\right)^2 &
\quad \text{subject to} \quad \sigma^{*}_{ijt}(\beta^{*}_i, \symbf{x_t}) = \frac{e^{\beta^{*}_i x_{jt} }}{1+\sum_{j'}e^{\beta^{*}_i x_{j't}}}\\
   0\leq  w_i \leq 1,& \quad   \sum_i w_i = 1
\end{align*}
\vspace{-0.5cm}
\begin{enumerate} 
    \item Draw a large number (thousands?) of $(\beta^{*}_i$ from a prior distribution $g(\beta_i)$ more dispersed than the true $f(\beta_i)$
    \item Compute individual choice probabilities $ \sigma^{*}_{ijt}(\beta^{*}_i)$
    \item Estimate above by constrained least squares (non-negative lasso)
    \item This produces sparse models. (most $w_i=0$)
\end{enumerate}
Fixed coefficients require EM. See Heiss, Hetzenecker, Osterhaus (JE 2022) for details (and elastic net variant $\sum_i w_i^2 \leq t$).
\end{frame}
...
Add unobservable error for each $s_{jt}$ labeled $\alert{\xi_{jt}}$.
\begin{align*}
...
Taking logs:
\begin{align*}
\ln \sigma_{0t} &= -\log \left(1+\sum_k \exp[x_{kt} \beta + \xi_{kt}] \right) \\
\ln \sigma_{jt} &= [x_{jt} \beta - \alpha p_{jt} +  \xi_{jt} ] - \log \left(1+\sum_k \exp[x_{kt} \beta + \xi_{kt}] \right)\\
\underbrace{\ln s_{jt}- \ln s_{0t}}_{\alert{Data!}} &= x_{jt} \beta -\alpha p_{jt} +  \xi_{jt}
\end{align*}
Exploit the fact that: 
\begin{enumerate}
\item Observed shares equal the model's choice probabilities: $s_{jt} = \sigma_{jt}$ (with no sampling error)
...
\ln s_{jt}- \ln s_{0t} &= x_{jt} \beta -\alpha p_{jt} +  \rho \log(s_{j|gt})  +  \xi_{jt}
\end{align*}
 \begin{itemize}
...
\begin{frame}
\frametitle{What about Zero Shares?}
Every inversion so far needs $\ln s_{jt}$ --- but real data are full of $s_{jt}=0$.
\begin{itemize}
\item Weekly store-level scanner data: most UPCs sell zero units in most store-weeks.
\item \alert{Sampling zeros}: finite number of consumers $N_t$; true $\sigma_{jt}>0$ but nobody happened to buy.
\item \alert{Structural zeros}: product genuinely not available (or not considered) in market $t$.
\item Common hacks and why they fail:
\begin{itemize}
\item Drop the zeros $\rightarrow$ \alert{selection on $\xi_{jt}$}: you keep only observations with favorable demand shocks; biases price coefficients toward zero.
\item Replace $s_{jt}$ with $s_{jt} + \epsilon$ or Laplace shares $\frac{n_{jt}+1}{N_t + J_t + 1}$ $\rightarrow$ arbitrary, and elasticities can be badly biased when zeros are common.
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Zeros: What can we do?}
\begin{itemize}
\item Gandhi, Lu, Shi (QE 2023): treat the true $\sigma_{jt}$ as bounded, construct moment inequalities/bounds that are robust to sampling zeros.
\item Dub\'e, Horta\c{c}su, Joo (2021): pairwise-differencing / GMM approach that avoids taking logs of zero shares.
\item Quan and Williams (RAND 2018): aggregate many small local markets (where the zeros live) to a level where shares are positive, modeling the heterogeneity you aggregate over.
\item Practical advice:
\begin{itemize}
\item Zeros are \alert{informative, not missing} --- think about \emph{why} the share is zero (availability? consideration? sampling?).
\item Aggregation (across weeks/stores) is often the honest fix.
\item If zeros are structural, model the choice set (product availability, \`a la Conlon and Mortimer (2013)).
\end{itemize}
\end{itemize}
\end{frame}
...
Think about a \alert{generalized inverse} for $\sigma_{j}(\symbf{\delta_{t}}, \symbf{x}_t,\theta_2) = s_{jt}$ so that 
\begin{align*}
%\sigma_{jt}^{-1}(\calS_{\cdot t},\widetilde{\theta}_2)+\alpha p_{jt}&= h(x_{jt},v_{jt},\theta_1)   + \xi_{jt}\\
 \sigma_{jt}^{-1}(\symbf{s}_t,\widetilde{\theta}_2)&= \delta_{jt} \equiv x_{jt} \beta -\alpha p_{jt} +  \xi_{jt} 
\end{align*}
 \begin{itemize}
\item After some transformation of data (shares $\symbf{s}_t$) we get \alert{mean utilities} $\delta_{jt}$.
% \begin{itemize}
...
\item Examples:
\begin{itemize}
\item Plain logit and nested logit: the inversions we just derived.
\item Three level nested logit: $\sigma_{j}^{-1}(\symbf{s}_t,\rho) = \ln s_{jt}- \ln s_{0t} -\sum_{d=1}^2 \rho_d \ln \left(\frac{s_{j t}}{s_{d(j), t}}\right)$ (Verboven 1996)
 \end{itemize}
 \item Anything with a share requires an IV.
 \end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Aside: The Pure Characteristics Model (Berry and Pakes 2007)}
Mixed logit \alert{without} the idiosyncratic taste shock:
\begin{align*}
u_{ijt} = \delta_{jt} + \mu_{ijt} + \underbrace{a}_{\rightarrow 0} \cdot\, \varepsilon_{ijt}
\end{align*}
\vspace{-0.5cm}
\begin{itemize}
\item Utility comes from characteristics only: vertical tastes over several dimensions (everyone prefers more MHz, more RAM; differ in WTP).
\item Substitution is purely \alert{local} in characteristic space: you compete only with nearby products; allows for \alert{crowding} of the product space.
\item Inversion/estimation much harder: no closed form for $\sigma_j$, and the share function is not smooth/invertible the way the logit's is (homotopy: Song; MPCC: Su) $\rightarrow$ why people avoid it in practice.
\item The logit $\varepsilon$ adds a ``cushion'': it guarantees full support (some substitution to \emph{every} product) --- but also mechanical CS gains from every new product.
\end{itemize}
\end{frame}


\begin{frame}
\frametitle{Aside: The Vertical Model (Bresnahan 1987)}
All consumers agree on the quality ranking; they differ only in willingness-to-pay for quality:
\begin{align*}
u_{ij} = \delta_j - \alpha_i p_j, \quad \text{sort } p_1 < \cdots < p_J \implies \delta_1 < \cdots < \delta_J \text{ (why?)}
\end{align*}
\vspace{-0.5cm}
\begin{itemize}
\item Analytic inversion via \alert{indifference cutoffs}: choose $j$ IFF
\begin{align*}
\frac{\delta_{j+1} - \delta_j}{p_{j+1} -p_j} < \alpha_i < \frac{\delta_j  - \delta_{j-1}}{p_j - p_{j-1}}, \quad
\sigma_j = F\left(\frac{\delta_j  - \delta_{j-1}}{p_j - p_{j-1}} \right) - F\left(\frac{\delta_{j+1} - \delta_j}{p_{j+1} -p_j} \right)
\end{align*}
where $F(\cdot)$ is the CDF of $\alpha_i$.
\item Each product competes \alert{only with its quality neighbors}: changing $p_j$ moves only $(\sigma_{j-1},\sigma_j,\sigma_{j+1})$; all other cross-elasticities are zero.
\item Original application: autos (Bresnahan's 1955 price war). If a luxury car and a truck have similar prices, expect strange substitution patterns.
\item Special case of pure characteristics with one dimension. Rarely used today.
\end{itemize}
\end{frame}
...
\begin{align*}
    \symbf{\delta^{(k)}}(\theta) = \symbf{\delta^{(k-1)}}(\theta) + \log(s_{jt}) - \log(\sigma_{j}(\symbf{\delta_t}^{(k-1)}, \theta)
\end{align*}
...
\begin{align*}
    \min_{\symbf{\delta}}  \sum_{i=1}^I w_i \cdot \log \left(\sum_{j} \exp\left(\delta_j  + \mu_{ij}(\theta_2) \right) \right) - \sum_k \delta_k \cdot s_{k}
\end{align*}
The first-order conditions are given by
$$\sigma_j(\symbf{\delta},\theta_2) = s_j$$
...
\begin{itemize}
 ...
 \begin{frame}
\frametitle{BLP Estimation: Pseudocode}
\footnotesize
The model is still defined by CMR $\mathbb{E}[\xi_{jt} \mid  z_{jt}^D]=0$. From the outside, in:
\begin{itemize}
\item Outer loop: search over nonlinear parameters $\theta_2$ to minimize GMM objective:
 \begin{align*}
 \widehat{\theta_{BLP}} = \arg \min_{\theta_2} (Z' \widehat{\xi}(\theta_2)) W  (Z' \widehat{\xi}(\theta_2))'
 \end{align*}
 \item Inner Loop:
 \begin{itemize}
\item Solve for $\delta$ so that $\sigma_{jt}(\delta,\theta_2) = s_{jt}$.
\begin{itemize}
\item Computing $\sigma_{jt}(\delta,\theta_2)$ requires numerical integration (quadrature or monte carlo).
\end{itemize}
 \item We can do IV-GMM to recover $\hat{\alpha}(\theta_2),\widehat{\beta}(\theta_2),\widehat{\xi}(\theta_2)$ from the change of variables:
  \begin{align*}
\delta_{jt}= x_{jt} \beta -\alpha p_{jt}+  \xi_{jt}
 \end{align*}
  \item Use $\widehat{\xi}(\theta_2)$ to construct moment conditions.
 \end{itemize}
...
\nonumber \log(s_{jt})  &= \log(\sigma_{jt}(\delta,\theta_2))
\end{align}
...
\nonumber \log(s_{jt})  &=& \log(\sigma_{jt}(\theta_2, \delta))
\end{eqnarray}
\end{frame}
...
\arg \max_{p \in \calJ_f} \pi_f (\symbf{p}) &= \sum_{j \in \calJ_f} (p_j - c_j) \cdot \sigma_j(\symbf{p}) +  \kappa_{fg}\sum_{k \in \calJ_g} (p_k - c_k) \cdot \sigma_k(\symbf{p}) \\
0&= \sigma_j(\symbf{p}) + \sum_{k \in \calJ_f} (p_k - c_k) \frac{\partial \sigma_{k}}{\partial p_j}(\symbf{p}) 
\end{align*}
}
...
\mathcal{H}(\kappa)_{(j,k)} = \left\{\begin{array}{lr}
      ...
      0 & \text{o.w}\\
        \end{array} \right\}
\end{align*}
We can re-write the FOC in matrix form where $\odot$ denotes Hadamard product (element-wise) and  $\Delta_{(j,k)}(\symbf{p})  = - \frac{\partial \sigma_{j}}{\partial p_k}(\symbf{p})$ is the demand derivatives matrix:
\begin{align*}
        \sigma(\symbf{p}) &= (\mathcal{H} \odot \Delta(\symbf{p})) \cdot (\symbf{p} - \symbf{mc}), \\
       \symbf{mc} &=  \symbf{p} - \underbrace{(\mathcal{H} \odot \Delta(\symbf{p}))^{-1} \sigma(\symbf{p})}_{\eta(\symbf{p},\symbf{s},\theta_2)}.
\end{align*}
\end{frame}
...
\footnotesize
\begin{enumerate}[(a)]
\item For each market $t$: solve $s_{jt} = \sigma_{jt}(\delta_{\cdot t},\theta_2)$ for $\widehat{\delta}_{\cdot t}(\theta_2)$.
...
\item Stack up $\widehat{\delta}_{\cdot t}(\theta_2)$ and $\widehat{mc}_{jt}(\widehat{\delta}_{\cdot t}(\theta_2),\theta_2)$ into the stacked system on the previous slide and use linear IV-GMM to recover $[\widehat{\theta}_1(\theta_2), \widehat{\theta}_3(\theta_2) ]$ following the recipe in Appendix of Conlon Gortmaker (2020)
\item Construct the residuals $\widehat{\xi}_{jt}(\theta_2)$ and $\widehat{\omega}_{jt}(\theta_2)$ from the stacked system.
...
\begin{column}{.5\textwidth}
  ...
  \item Plain logit gives $\epsilon_{jj} = \alpha \cdot p_j  \cdot (1-\sigma_j)$.
  \end{itemize}
  \end{column}
\end{columns}
\end{frame}
...
\vspace{-0.3cm}
    \begin{center}
    \scalebox{0.55}{
    \input{resources/table3_parameter_results.tex}
    }
    \end{center}
  \end{column}
  \hfill
...
\begin{column}{.55\textwidth}
  \begin{itemize}
    \item Demographic Interactions w/ 5 income bins \\ (matched to micro-moments: defined properly next lecture)
...
\begin{center}
\scalebox{0.6}{
\input{resources/table4_diversion_results.tex}
}
\end{center}
% \begin{tablenotes}
...
\begin{eqnarray*}
    \delta_{jt}(\symbf{s}_t,\alert{\textrm{y}_t}, \widetilde{\theta}_2) &=&  [\textrm{x}_{jt}, \alert{\textrm{v}_{jt}}]  \beta  - \alpha p_{jt} + \xi_{jt}\\
...
\begin{eqnarray*}
\frac{\sigma_{jt}(\symbf{p_t})}{\left|\frac{\partial \sigma_{jt}(\symbf{p_t})}{\partial p_{jt}}\right|} = \frac{1}{\alpha} \frac{1}{1-\sigma_{jt}} \rightarrow \frac{1}{\alpha}
\end{eqnarray*}
\begin{itemize}
...
\begin{itemize}
\item Also need instruments for the $\Sigma$ random coefficient parameters.
...
\begin{equation}
    \label{eq:frac}
    \log\frac{s_{jt}}{s_{0t}} \approx \beta_0 x_{jt} + \Sigma_0^2 a_{jt} + \Pi_0 m_t^y x_{jt} + \Pi_0^2 v_t^y a_{jt} + \xi_{jt}, \quad a_{jt} = \Big(\frac{x_{jt}}{2} - \sum_{k \in \calJ_t} s_{kt} \cdot x_{kt}\Big) \cdot x_{jt}
\end{equation}
\begin{itemize}
...
\begin{align*}
\frac{1}{2} x_{jt}^2 - 2 x_{jt} \cdot \sum_k s_{kt}  \cdot x_{kt}
\end{align*}
\begin{itemize}
...
g(x_i,v_i,z_i) &= \left(y_i - \beta x_i - \gamma v_i \right)
 \end{align*}
 Which gives:
\begin{align*}
...
\item $\E\left[\frac{\partial \xi_{jt}}{\partial \widetilde{\theta}_2} \mid X_t, w_{t} \right] =\E\left[\left[\frac{\partial \symbf{\sigma_t}}{\partial \symbf{\delta_t}}\right]^{-1} 
\left[\frac{\partial \symbf{\sigma_t}}{\partial \widetilde{\theta}_2}\right] \mid Z_{jt}^D \right]$  (not conditioned on endogenous $p$!)
\end{enumerate}
...
\item Solve shares $\symbf{\sigma_{t}}(\symbf{\hat{p}_{t}},\widehat{\theta})$
...
\begin{eqnarray*}
c = p  + \underbrace{\Delta^{-1} \sigma}_{\rightarrow 0}  = X \gamma_1 + W \gamma_2 + \omega
\end{eqnarray*}
...
\item Turns out that the important piece tends to be \alert{shape} of jacobian for $\Sigma$.
...
\begin{frame}{IV Comparison: Conlon and Gortmaker (2020): Results}
\begin{center}
...
\begin{itemize}
\item The unrestricted matrix of $D_{jk}(\symbf{p})$'s (or elasticities) is $J \times J$ and probably not feasible to estimate directly. $\rightarrow$ need some \alert{dimension reduction}.
\item Logit simply assumes proportional substitution ${D}_{jk}=\frac{\sigma_k}{1-\sigma_{j}}$ so that $\symbf{D}(\symbf{p})$ is of rank one!
...
\begin{itemize}
\item Plain logit assumes \alert{elasticities increasing in $p_j$} and \alert{markups that are decreasing}.\\
\begin{itemize}
...
\begin{frame}{What we assumed away: Search and Consideration}
\begin{itemize}
\item Everything this week assumed \alert{full information} and \alert{full consideration}: every consumer evaluates every $j \in \calJ_t$.
\item If consumers only consider $C_i \subset \calJ$, estimated elasticities conflate \alert{preferences} with \alert{attention}:
\begin{itemize}
\item Goeree (2008): PCs --- advertising and limited information.
\item Honka (2014): search and switching costs in auto insurance.
\item Honka, Horta\c{c}su, Vitorino (2017): advertising and awareness in banking.
\item Abaluck and Adams-Prassl (2021): recovering consideration from asymmetries in demand (menus).
\end{itemize}
\item With search frictions, price responses mix ``who searches'' with ``who prefers'' --- a different demand object.
\item Search and consideration get several weeks of their own in ECON-GA 1802 (spring).
\end{itemize}
\end{frame}
