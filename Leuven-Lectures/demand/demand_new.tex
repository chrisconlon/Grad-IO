...
% extend graphicspath: supply_table/supply_figure live in the Week 4 resources
\graphicspath{{./}{./resources/}{../resources/}{../../resources/}{../../Week 4- BLP Instruments and Identification/}}
...
\begin{align*}
(d_{i1},\ldots,d_{iJ},d_{i0}) \sim \text{Categorical} (\sigma_{i1},\ldots,\sigma_{iJ},\sigma_{i0} ) 
\end{align*}
\end{frame}
...
\begin{align*}
\sigma_{ij}(\theta)= \mathbb{P}(d_{ij}=1) = \int \frac{\exp[\beta_{\iota} \, x_j]}{1+\sum_{k \in \calJ} \exp[\beta_{\iota} \, x_k]}\, f(\beta_{\iota} \mid \theta)\, \partial \beta_{\iota}
\end{align*}
Which gives us the log-likelihood:
 \begin{align*}
 \ell(\theta) = \sum_{i=1}^N \sum_{j \in \calJ \cup \{0\}} d_{ij} \log \sigma_{ij}(\symbf{x_i} \mid \theta)
 \end{align*}
...
\item Examples:
\begin{itemize}
\item Plain Logit: $\sigma_{j}^{-1}(\symbf{s}_t) = \ln s_{jt}- \ln s_{0t}$
\item Nested Logit: $\sigma_{j}^{-1}(\symbf{s}_t,\rho) = \ln s_{jt}- \ln s_{0t} - \rho  \ln s_{j|gt}$
...
\item Outer loop: search over nonlinear parameters $\theta$ to minimize GMM objective:
 ...
 \begin{itemize}
% \item Fix $\theta$.
\item Solve for $\delta$ so that $\sigma_{jt}(\delta,\theta) = s_{jt}$.
\begin{itemize}
\item Computing $\sigma_{jt}(\delta,\theta)$ requires numerical integration (quadrature or monte carlo).
\end{itemize}
...
It is helpful to define the \alert{demand derivatives matrix} $\Delta_{(j,k)}(\symbf{p})  = - \frac{\partial \sigma_{j}}{\partial p_k}(\symbf{p})$, and the \alert{ownership matrix}:
\begin{align*}
...
\item $\mathbb{E}\left[\frac{\partial \xi_{jt}}{\partial \widetilde{\theta}_2} \mid X_t, w_{t} \right] =\mathbb{E}\left[\left[\frac{\partial \symbf{\sigma_t}}{\partial \symbf{\delta_t}}\right]^{-1} 
