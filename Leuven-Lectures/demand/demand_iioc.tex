...
%\usepackage{natbib}
% dutchcal removed: its legacy \mathcal glyphs crash XeTeX under teaching_slides'
% unicode-math when combined with \hat{...}; unicode-math's script alphabet is used instead
...
\begin{align*}
\symbf{mc}(\theta_2)&\equiv \symbf{p}- \symbf{\eta}(\symbf{s}_t,\symbf{p}_t,\chi_t, y_t, ;\theta_2) \\
f\left(\symbf{p}- \symbf{\eta}(\symbf{s}_t,\symbf{p}_t,\chi_t, y_t, ;\theta_2)  \right) &= [\textrm{x}_{jt} \,, \textrm{w}_{jt}] \theta_3 + \alert{g(q_{jt})} +  \omega_{jt}
\end{align*}
...
\begin{align*}
\sigma_j^{-1}(\symbf{s}_t,\symbf{p}_t,\chi_t, y_t; \theta_2) &= [\mathrm{x}_{jt} \,,  \mathrm{v}_{jt}]\, \theta_1 - \alpha p_{jt} + \xi_{jt}\\
f\left(p_{jt}- \eta_{jt}(\symbf{s}_t,\symbf{p}_t,\chi_t, y_t ;\theta_2)  \right) &= [\mathrm{x}_{jt} \,, \mathrm{w}_{jt}]\, \theta_3 +  \omega_{jt}
\end{align*}
...
\item Solve firm FOC's for $\symbf{\hat{p}_{t}}(\symbf{\xi}^{*},\symbf{\omega}^{*},\widehat{\theta})$ and shares $\symbf{\sigma_{t}}(\symbf{\hat{p}_{t}},\widehat{\theta})$
...
Like a one-step version of Goolsbee Petrin (2004)
        ...
        \begin{enumerate}
            \item the Mixed Data Likelihood Estimator:
                \begin{align*}
                    \log\hat{L}(\theta,\delta) =\sum_{m=1}^M\sum_{j=0}^{J_m}\sum_{i=1}^{N_m} d_{ijm}(D_{im} \underbrace{\log \sigma_{jm}^{y_{im}}(\theta_2,\delta)}_{\text{individual choices}} + (1-D_{im}) \underbrace{\log\sigma_{jm}(\theta_2,\delta)}_{\text{aggregate share}})%\\
...
\item Three level nested logit: $\sigma_{j}^{-1}(\symbf{s}_t,\rho) = \ln s_{jt}- \ln s_{0t} -\sum_{d=1}^2 \rho_d \ln \left(\frac{s_{jt}}{s_{d(j), t}}\right)$ (Verboven 1996)
\item IPDL: $\sigma_{j}^{-1}(\symbf{s}_t,\rho) = \ln s_{jt}- \ln s_{0t}-\symbf{x}_{j t} \symbf{\beta}-\alpha p_{j t}-\sum_{d=1}^D \rho_d \ln \left(\frac{s_{jt}}{s_{d(j), t}}\right)+\xi_{j t}$\\ (Fosgerau, Monardo, De Palma (2022))
 \end{itemize}
...
\begin{align*}
    \log\frac{s_{jt}}{s_{0t}} \approx \beta_0 x_{jt} + \Pi_0 m_t^y x_{jt} + \left(\Sigma_0^2+  \Pi_0^2 v_t^y \right) a_{jt} + \xi_{jt}, \quad a_{jt} = \Big(\frac{x_{jt}}{2} - \sum_{k \in \calJ_t} \alert{s_{kt}} \cdot x_{kt}\Big) \cdot x_{jt}
\end{align*}
\begin{itemize}
...
\begin{align*}
\frac{1}{2} x_{jt}^2 - \alert{2 x_{jt} \cdot \sum_k s_{kt}  \cdot x_{kt}}
\end{align*}
\begin{itemize}
...
\small
Fix $\textrm{rank}(\symbf{D}(\symbf{S},w))=I$, and for each choice of $I$ solve:
\begin{align*}
\min _{(\symbf{S}, w) \geq 0} \left\|\mathcal{P}_{\Omega}(\calD-\symbf{D}(\symbf{S},w))\right\|_{\ell_2}  &+ \lambda \left\|\symbf{s}- \symbf{S}\,w \right\|_{\ell_2} \text { with } \left\|w  \right\|_{\ell_1} \leq 1, \quad   \left\|\symbf{\sigma_i}  \right\|_{\ell_1} \leq 1.
\end{align*}
\vspace{-.25cm}
\begin{itemize}
\item Goal: estimate $\symbf{\sigma_i}$ (choice probabilities) and corresponding weights $w_i$ (Finite Mixture) in \alert{product space}
\begin{itemize}
\item Consistent with $U_{ij} = V_{ij} + \varepsilon_{ij}$ and logit error.
\end{itemize}
\item Constraints: Choice probabilities $\sigma_{ij}$ sum to one, type weights $w_i$ sum to one.
\begin{itemize}
...
\end{itemize}
\item Matrix completion: We can construct estimates of $\symbf{D}(\symbf{S},w)$ including elements of $\mathcal{P}_{\overline{\Omega}}$.
