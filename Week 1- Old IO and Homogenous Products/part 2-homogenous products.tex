...
\begin{itemize}
\item For most of you, this was probably a review: we know the solution is an \alert{excluded instrument} that shifts one curve without affecting the other, which we can use to form a 2SLS estimate.
\item Instead let's look at something a little different: Angrist, Imbens, and Graddy (ReStud 2000).
\begin{itemize}
...
\begin{frame}{AIG: Negative Results}
\begin{itemize}
...
\begin{frame}{AIG: Structural Interpretation}
...
\begin{frame}{AIG: Nonlinear IV}
\small
The Wald estimand is a weighted average of demand slopes:
\begin{eqnarray*}
\alpha_{1,0} &=& \frac{\E [ \int_{\tilde{p}(0,t)}^{\tilde{p}(1,t)} \frac{\partial q_t^d(s)}{\partial s} d\, s]}{\E \tilde{p}(1,t)- \E\tilde{p}(0,t)}
 \,\rightarrow\, \int_0^{\infty} \E\left[ \frac{\partial q_t^d(s)}{\partial s} \,\Big|\, s \in [\tilde{p}(0,t),\tilde{p}(1,t)]\right ] \omega(s)\, d  s
\end{eqnarray*}
\begin{itemize}
\item Given $t$: average the slope of $q_t^d$ from $\tilde{p}(0,t)$ to $\tilde{p}(1,t)$. Given price $s$: average across $t$ (randomness is due to $\epsilon_t$).
...
\item Case (2): $q_t^d(p) = \alpha_{0t} + \alpha_{1t} p + \epsilon_t$ gives
$\alpha_{1,0} = \frac{\E[\alpha_{1t} (\tilde{p}(1,t) - \tilde{p}(0,t))]}{\E \tilde{p}(1,t) - \E \tilde{p}(0,t)} \neq \E [\alpha_{1,t}]$: we need \alert{mean independence}.
\item With a continuous $z$ we can do a full nonparametric IV estimator: kernel-estimate $\E(q_t|z)$ and $\E(p_t|z)$ and take the ratio of derivatives $\alpha'(z) = \hat{q}'(z)/\hat{p}'(z)$ (a local Wald estimate at each $z$).
\end{itemize}
\end{frame}
...
\item When you have a parametric model, you don't need these results because we can define whatever (nonlinear) parametric functional form we want. (Parsimonious and realistic parametric functional forms are the rest of the course.)
\item Caveat: this only works for a single product --- with multiproduct oligopoly it is much harder to satisfy the \alert{monotonicity} condition. (Why?)
\end{itemize}
\end{frame}
