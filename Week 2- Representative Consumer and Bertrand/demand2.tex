...
%   \end{frame}
% }
\title{Week 2: Demand Estimation (Part two) [OLD]}
\author{Chris Conlon}
\institute{Grad IO}
\date{\today}
...
\begin{frame}{Reading for Today}
These notes are based on Ken Train's book \textit{Discrete Choice Methods with Simulation}
...
\begin{eqnarray*}
\sigma_{ij} = \Pr( U_{ij} > U_{ik} \quad \forall j \neq k)
\end{eqnarray*}
\end{frame}
...
\begin{eqnarray*}
\sigma_{ij} &=& \Pr( U_{ij} > U_{ik} \quad \forall j \neq k)\\
 &=& \Pr( V_{ij} + \varepsilon_{ij} > V_{ik} + \varepsilon_{ik} \quad \forall j \neq k)\\
 &=& \Pr( \varepsilon_{ij}-\varepsilon_{ik} > V_{ik} - V_{ij} \quad \forall j \neq k)
\end{eqnarray*}
\pause
It is helpful to define $f(\varepsilon_{i})$ as the $J$ vector of individual $i$'s unobserved utility.
\begin{eqnarray*}
\sigma_{ij} &=& \Pr( \varepsilon_{ij}-\varepsilon_{ik} > V_{ik} - V_{ij} \quad \forall j \neq k)\\
...
\begin{eqnarray*}
\sigma_{ij} &=&  \int I( \varepsilon_{ij}-\varepsilon_{ik} > V_{ik} - V_{ij} ) f( \varepsilon_i) \partial \varepsilon_i 
\end{eqnarray*}
...
\small
\begin{itemize}
\item Only differences in utility matter: $\Pr( \varepsilon_{ij}-\varepsilon_{ik} > V_{ik} - V_{ij} \quad \forall j \neq k)$
...
Consider the case where $Var(\varepsilon_{ij}^B) = a^2$ and   $Var(\varepsilon_{ij}^C) =  k^2 a^2$ :
\begin{itemize}
...
\begin{eqnarray*}
\sigma_{ij} = \frac{e^{V_{ij}}}{\sum_k e^{V_{ik}}} \approx \frac{e^{\beta' x_{ij}}}{\sum_k e^{\beta' x_{ik}}}
\end{eqnarray*}
...
\begin{eqnarray*}
\log \frac{\sigma_{ij}(\theta)}{\sigma_{ik}(\theta)} = \symbf{x_{ij}} \beta_j - \symbf{x_{ik}} \beta_k \rightarrow  \symbf{x_i}\cdot(\beta_j - \beta_k)
\end{eqnarray*}
...
\begin{eqnarray*}
\sigma_{ij} = \frac{\exp[\symbf{x_i}\beta_j ]}{1+\sum_k \exp[\symbf{x_i} \beta_k ]} 
\end{eqnarray*}
\begin{itemize}
...
\begin{itemize}
\item Consider $U_{ij}^{*} = V_{ij} + \varepsilon_{ij}^{*}$ with $Var(\varepsilon^{*}) = a^2 \pi^2/6$.
\item Without changing behavior we can divide by the scale parameter $a$ so that $U_{ij} = V_{ij}/a + \varepsilon_{ij}$ and $Var(\varepsilon^{*}/a)=Var(\varepsilon) = \pi^2/6$
\begin{eqnarray*}
...
\item Ratio $\beta_1/\beta_2$ is invariant to the scale parameter $a$.
\end{itemize}
\end{frame}
...
\begin{frame}{Taste Variation}
\begin{eqnarray*}
\frac{\sigma_{ij}}{\sigma_{ik}} = \frac{e^{V_{ij}}}{\sum_{k'} e^{V_{ik'}}} / \frac{e^{V_{ik}}}{\sum_{k'} e^{V_{ik'}}} = \frac{e^{V_{ij}}}{e^{V_{ik}}} = \exp[V_{ij} - V_{ik}].
\end{eqnarray*}
\begin{itemize}
...
\begin{itemize}
\item The well known counterexample: You can choose to go to work on a car $c$ or blue bus $bb$. $\sigma_{c} = \sigma_{bb} = \frac{1}{2}$ so that $\frac{\sigma_c}{\sigma_{bb}} = 1$.
...
And Elasticity:
\begin{eqnarray*}
\frac{ \partial \log \sigma_{ij}}{ \partial \log z_{ij}} = \sigma_{ij}(1- \sigma_{ij}) \frac{\partial V_{ij}}{\partial z_{ij}} \frac{z_{ij}}{\sigma_{ij}} = (1- \sigma_{ij}) z_{ij} \frac{\partial V_{ij}}{\partial z_{ij}}
\end{eqnarray*}
With cross effects:
\begin{eqnarray*}
\frac{\partial \sigma_{ij}}{\partial z_{ik}} = -\sigma_{ij} \sigma_{ik} \frac{\partial V_{ik}}{\partial z_{ik}}
\end{eqnarray*}
And Elasticity:
\begin{eqnarray*}
\frac{ \partial \log \sigma_{ij}}{ \partial \log z_{ik}} = -\sigma_{ik} z_{ik} \frac{\partial V_{ik}}{\partial z_{ik}}
\end{eqnarray*}
...
\item Likewise removing an option $k$ means that $\tilde{\sigma}_{ij} = \frac{\sigma_{ij}}{1-\sigma_{ik}}$ for all other $j$.
...
\begin{eqnarray*}
L(\symbf{y} | \symbf{x}, \theta) &=& \prod_{i=1}^N  \underbrace{\frac{n_i!}{\prod_{j=1}^J y_{ij}!}}_{C(\symbf{y})} \prod_{j=1}^J  \sigma_{ij}(x_{ij},\theta)^{y_{ij}} \\
ll(\symbf{y} | \symbf{x}, \theta) &=& \sum_{i=1}^N \log(C(\symbf{y}))   + \sum_{i=1}^N \sum_{j=1}^J y_{ij} \log( \sigma_{ij}(x_{ij},\theta)) \\
l(\symbf{y} | \symbf{x}, \theta) &\approx& \sum_{i=1}^N \sum_{j=1}^J y_{ij} \log( \sigma_{ij}(x_{ij},\theta))
\end{eqnarray*}
\begin{itemize}
...
\item The point of the inclusive value is to guarantee that $\sum_{k=1} \sigma_{ik}(\symbf{x_i},\theta) = 1$.
...
\begin{eqnarray*}
l(\symbf{y} | \symbf{x}, \theta) &\approx& \sum_{j=1}^J  y_{j}^{*} \log( \sigma_{j}(\symbf{x},\theta))
\end{eqnarray*}
...
\item All of this depends on the precise form of $ \sigma_{j}(\symbf{x_i},\theta)$. When it doesn't change across observations: we can aggregate.
\item It functions as if we have a representative consumer up to $\varepsilon_{i}$.
\item We can use this idea to go from individual level to market demand: $q_j(\symbf{x_i}) = N_i \sigma_{ij}(\theta)$.
\end{itemize}
\end{frame}
...
\begin{itemize}
\item An important element in $\symbf{x_i}$ are prices $[p_{1},\ldots,p_J]$
\item Helpful to write $u_{ij} = x_j \beta -\alpha p_j$ (assumes aggregation!).
\begin{eqnarray*}
\frac{\partial q_j}{\partial p_k} =-N \cdot \alpha \left(I[j = k] \sigma_j -\sum_{k=1}^K \sigma_j \sigma_k \right)
\end{eqnarray*}
\item This implies that $ \eta_{jj} = \frac{\partial q_j}{\partial p_j} \frac{p_j}{q_j}  = -\alpha p_j (1-\sigma_j)$.
\item The price elasticity is increasing in own price! (Why is this a bad idea?)
\item  $ \eta_{jk} = \frac{\partial q_j}{\partial p_k} \frac{p_k}{q_j}  = -\alpha p_k \sigma_k$.
...
\begin{itemize}
\item Suppose we got rid of a product $k$ then $\sigma_j^{(1)} = \sigma_j^{(0)} \frac{1}{1- \sigma_k}$.
\item Substitution is just proportional to your pre-existing shares $\sigma_j$
...
\item Each time we want to compute $\sigma_j(\theta)$ we have to simulate an integral of dimension $J$.
...
\begin{eqnarray*}
\sigma_{ij} = \frac{ e^{V_{ij}/\lambda_g} \left(\sum_{k \in J_g} e^{V_{ik}/\lambda_g} \right)^{\lambda_g -1}}{\sum_{h=1}^G \left(\sum_{k \in J_h} e^{V_{ik}/\lambda_h} \right)^{\lambda_h}}
\end{eqnarray*}
Within the same group $g$ we have IIA and proportional substitution 
\begin{eqnarray*}
\frac{\sigma_{ij}}{\sigma_{ik}} = \frac{ e^{V_{ij}/\lambda_g}}{ e^{V_{ik}/\lambda_g}}
\end{eqnarray*}

But for different groups we do not:
\begin{eqnarray*}
\sigma_{ij} = \frac{ e^{V_{ij}/\lambda_g} \left(\sum_{k \in J_g} e^{V_{ik}/\lambda_g} \right)^{\lambda_g -1}}{ e^{V_{ik}/\lambda_h} \left(\sum_{k \in J_h} e^{V_{ik}/\lambda_h} \right)^{\lambda_h -1}}
\end{eqnarray*}
\end{frame}
...
\begin{eqnarray*}
\sigma_{ij} &=& \frac{ e^{V_{ij}/\lambda_g}}{ \left(\sum_{k \in J_g} e^{V_{ik}/\lambda_g} \right)}
\cdot
\frac{ \left(\sum_{k \in J_g} e^{V_{ik}/\lambda_g} \right)^{\lambda_g}}{\sum_{h=1}^G \left(\sum_{k \in J_h} e^{V_{ik}/\lambda_h} \right)^{\lambda_h}} \\
&=& \underbrace{\frac{ e^{V_{ij}/\lambda_g}}{ \left(\sum_{k \in J_g} e^{V_{ik}/\lambda_g} \right)}}_{\sigma_{ij|g}}
\cdot
\underbrace{\frac{e^{\lambda_g IV_{ig}}}{\sum_{h=1}^{G} e^{\lambda_h IV_{ih}} }}_{\sigma_{ig}}
\end{eqnarray*}
...
\begin{eqnarray*}
\sigma_{ij} = \sigma_{ij|g}(\theta) \sigma_{ig}(\theta)
\end{eqnarray*}
\begin{itemize}
\item Because the nested logit can be written as the within group share $\sigma_{ij|g}$ and the share of the group $\sigma_{ig}$ we often explain this model as \alert{sequential choice}
...
\begin{eqnarray*}
 ...
 \sigma_{ij|g}(\theta) &=& \frac{\exp [x_j \beta/(1-\lambda_g)]}{\sum_{k \in G} \exp [x_k \beta/(1-\lambda_g)]} \\
 \sigma_{ig}(\theta) &=& \frac{\exp [IV_{ig}]^{1-\lambda_g}}{\sum_{h} \exp [IV_{ih}]^{1-\lambda_h}} 
\end{eqnarray*}
%\begin{itemize}
...
How does it actually look?
\begin{eqnarray*}
\log \left(\frac{\sigma_{ij|g}(\theta)}{\sigma_{ik|g}(\theta)} \right) = (x_j -x_k)\cdot \frac{\beta}{1-\lambda_g}
 \end{eqnarray*}
 \begin{itemize}
...
\begin{frame}
\frametitle{Parametric Identification (Ackerberg and Rysman RJE)}

Look at derivatives:
\begin{eqnarray*}
\frac{\partial\, \sigma_{j|g}}{\partial X_j} &=& \beta_1 \sigma_{j|g}(1-\sigma_{j|g}) \\
 \frac{\partial\, \sigma_{g}}{\partial X} &=& (1-\lambda) \beta_1 \sigma_{g}(1-\sigma_{g}) \\
  \frac{\partial\, \sigma_{g}}{\partial J} &=& \frac{1-\lambda}{J} \sigma_{g}(1-\sigma_{g})
\end{eqnarray*}
\begin{itemize}
...
\sigma_{ij} &=& \int \frac{\exp[x_{j} \beta + \mu_{ij} ]}{1+\sum_k \exp[x_{k} \beta + \mu_{ik} ]} f(\mu_i | \theta)
\end{eqnarray*}
 \begin{itemize}
...
How does it work?
 \begin{itemize}
\item Well we are mixing over individuals who conditional on $\beta_i$ or $\mu_i$ follow logit substitution patterns, however they may differ wildly in their $\sigma_{ij}$ and hence their substitution patterns.
...
\begin{eqnarray*}
\sigma_{ij}(\theta) = \int \frac{ e^{V_{ij}(\nu_i,\theta)}}{\sum_k e^{V_{ik}(\nu_i,\theta)}} f(\nu_i) \partial \nu
\end{eqnarray*}
\item Monte Carlo Integration: Independent Normal Case
\begin{itemize}
\item Draw $\nu_i$ from the standard normal distribution.
\item Now we can rewrite $\beta_i = \overline{\beta} + \Sigma \nu_i$
\item For each $\beta_i$ calculate $\sigma_{ij}(\beta_i)$.
\item $\frac{1}{S} \sum_{s=1}^S \sigma_{ij} = \widehat{\sigma_{j}^{s}}$
\end{itemize}
...
\begin{eqnarray*}
\max_{\theta} \sum_{i=1}^N y_{ij} \log \sigma_{ij}(\theta)
\end{eqnarray*}
...
\item The tough part is generally working out what $\frac{\partial \log \sigma_{ij}}{\partial \theta}$ is, especially when we need to simulate to obtain $\sigma_{ij}$.
...
\item One wrinkle--how do we compute the integral?
 \end{itemize}
\begin{eqnarray*}
\sigma_{ij} &=& \int \frac{\exp[x_{j} \beta_i  ]}{1+\sum_k \exp[x_{k} \beta_i  ]} f(\beta_i | \theta) \\
...
\item Compute $\hat{\sigma}_{ij}(\beta_s)$ for each draw of $\beta_s$ and each $j$.
\item Holding $\hat{\sigma}_{ij}(\beta_s)$ fixed, look for $w_s$ that solve
\begin{eqnarray*}
\min_w \left(s_j -  \sum_{s=1}^{ns} w_s \hat{\sigma}_{ij}(\beta_s) \right)^2 \quad \mbox{ s.t. } \sum_{s=1}^{ns} w_s = 1, \quad w_s \geq 0 \quad \forall s
\end{eqnarray*}
\end{itemize}
\end{frame}
...
\begin{eqnarray*}
\min_{\theta} \sum_j q_j \ln \sigma_j(\theta) \quad \mbox{s.t.} \quad  \sigma_j(\theta) = \frac{e^{x_j \beta/ \lambda}( \sum_{k \in g_l} e^{x_j \beta/ \lambda})^{\lambda-1}}{\sum_{\forall l'} ( \sum_{k \in g_l'} e^{x_j \beta/ \lambda})^{\lambda} }
\end{eqnarray*}
This is a pain to show but the problem is with the cross term $\frac{\partial^2 \sigma_j}{\partial \beta \partial \lambda}$ because $\exp[x_j \beta / \lambda]$ is not convex.
\end{exampleblock}
\begin{exampleblock}{A Simple Substitution Saves the Day:  let $\gamma = \beta / \lambda$}
\begin{eqnarray*}
\min_{\theta} \sum_j q_j \ln \sigma_j(\theta) \quad \mbox{s.t.} \quad  
\sigma_j(\theta) = \frac{e^{x_j \gamma}( \sum_{k \in g_l} e^{x_j \gamma})^{\lambda-1}}{\sum_{\forall l'} ( \sum_{k \in g_l'} e^{x_j \gamma})^{\lambda} }
\end{eqnarray*}
