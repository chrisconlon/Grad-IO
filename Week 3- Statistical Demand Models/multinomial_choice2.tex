...
Recall: logit's IIA means proportional substitution --- can we do better? Multinomial Probit?
\begin{itemize}
...
\begin{frame}{Nested Logit: Tree Structure}
\begin{figure}[htbp]
\begin{center}
...
\begin{eqnarray*}
\frac{\sigma_{ij}}{\sigma_{ik}} = \frac{ e^{V_{ij}/\lambda_g} \left(\sum_{k \in J_g} e^{V_{ik}/\lambda_g} \right)^{\lambda_g -1}}{ e^{V_{ik}/\lambda_h} \left(\sum_{k \in J_h} e^{V_{ik}/\lambda_h} \right)^{\lambda_h -1}}
\end{eqnarray*}
\end{frame}
...
\item Because the nested logit can be written as the within group share $\sigma_{ij|g}$ and the share of the group $\sigma_{ig}$ we often explain this model as \alert{sequential choice}. It could just be a \alert{block structure} on $\varepsilon_i$.
...
Look at derivatives:
\begin{align*}
\frac{\partial\, \sigma_{ij|g}}{\partial X_j} &= \beta_x \cdot \sigma_{ij|g} \cdot (1-\sigma_{ij|g}) \\
 \frac{\partial\, \sigma_{ig}}{\partial X} &= (1-\lambda_g) \cdot \beta_x \cdot \sigma_{ig}(1-\sigma_{ig}) \\
  \frac{\partial\, \sigma_{ig}}{\partial J} &= \frac{1-\lambda_g}{J} \cdot \sigma_{ig} \cdot (1-\sigma_{ig})
\end{align*}
\begin{itemize}
...
An alternative version of the nested logit is popular in IO (Cardell 1991) with nesting parameter $\rho \approx 1-\lambda$ (beware: Berry 1994 and much of IO write $\sigma$ for what we call $\rho$):
\begin{align*}
\sigma_{ij|g}&=\frac{e^{V_{ij}/(1-\rho)}}{D_{ig}}  \quad &
D_{ig}&= \sum_{j \in \mathcal{G}} e^{V_{ij}/(1-\rho)}   \\
\sigma_{ig}&=\frac{D_{ig}^{(1-\rho)}}{\sum_{g} D_{ig}^{(1-\rho)}} \quad &
\sigma_{ij}=  \sigma_{ij|g}\cdot \sigma_{ig} &=\frac{\exp \left(\frac{V_{ij}}{1-\rho}\right)}{D_{g}^{\rho}\left[\sum_{g} D_{g}^{(1-\rho)}\right]}
\end{align*}
...
It is helpful to define: $Z(\rho,\sigma_g)=[\rho + (1-\rho) \sigma_g ] \in (0,1]$ and note that $Z(0,\sigma_g)=\sigma_g$ and $Z(1,\sigma_g)=1$. If two products are in the same nest or different nests respectively:
\begin{align*}
 -\frac{\frac{\partial \sigma_{k}}{\partial p_j}}{\frac{\partial \sigma_{j}}{\partial p_j} } \text{| same}
&=\frac{ \sigma_{k|g} }{ Z^{-1}(\rho,\sigma_g) - \sigma_{j|g}}  \equiv D_{jk}^{*}\\
 -\frac{\frac{\partial \sigma_{k}}{\partial p_j}}{\frac{\partial \sigma_{j}}{\partial p_j} } \text{| different}
&= \frac{ \sigma_{k}  (1-\rho) }{ 1 - \sigma_{j|g} \cdot Z(\rho,\sigma_{g(j)}) } \equiv D_{jk}^{**}
\end{align*}
These are related by:
\begin{align*}
D_{jk}^{**}&=  D_{jk}^{*} \cdot \frac{\sigma_{g(k)} \cdot (1-\rho)}{Z(\rho,\sigma_{g(j)})}
\end{align*}
\end{frame}
...
\item Trade/Macro cousin: \alert{nested CES} --- the same segment-level substitution logic in a representative-agent package (recall the CES-logit equivalence from part 1).
 \end{itemize}
\end{frame}
...
\begin{align*}
\sigma_{ij}(\calJ)=\frac{y_{ij}\cdot  \frac{\partial G_i}{\partial y_{j}}\left(y_{i1}, \ldots, y_{iJ}\right)}{\mu \cdot G\left(y_{i1}, \ldots, y_{iJ}\right)}
\end{align*}
...
\begin{align*}
\sigma_{ij}(\calJ)=\frac{y_{ij}\cdot  \frac{\partial G_i}{\partial y_{j}}\left(y_{i1}, \ldots, y_{iJ}\right)}{\mu \cdot G_i\left(y_{i1}, \ldots, y_{iJ}\right)}
\end{align*}
\begin{itemize}
