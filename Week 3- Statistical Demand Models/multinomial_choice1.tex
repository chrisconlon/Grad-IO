...
\begin{align*}
\log \frac{s_{ij}(\theta)}{s_{ik}(\theta)}  = \frac{e^{V_ij}}{e^{V_{ik}}} = e^{V_{ij} - V_{ik}}
\end{align*}
...
\item Likewise removing an option $k$ means that $\tilde{s}_{ik}(\mathcal{J} \setminus j) = \frac{s_{ik}}{1-s_{ij}}$ for all other $k$.
