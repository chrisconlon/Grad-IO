...
\begin{align*}
s_{ij}(\theta) &= \int \frac{\exp[x_{j} \beta_i ]}{1+\sum_k \exp[x_{k} \beta_i ]} f(\symbf{\beta_i} | \theta) \approx \sum_{s=1}^S w_i^s \, \frac{\exp[x_{j} \beta_i^s ]}{1+\sum_k \exp[x_{k} \beta_i^s ]}
\end{align*}
