...
\begin{align*}
\int \cdots \int \prod_t \symbf{1}(i_t^{*} \left(\varepsilon_t,\theta) = i_t\right) \prod_t d F(\varepsilon_t | \varepsilon_{t-1}; \theta)
\end{align*}
...
\begin{align*}
\ell(i_t,\ldots,i_T | \varepsilon_0,i_0;\theta) \approx \frac{1}{S} \sum_s \prod_{t=1}^T \symbf{1}(i_t^s = i_t)
\end{align*}
...
\begin{eqnarray*}
l(y_t | \epsilon^t , y^{t-1}) = p(i_t | \epsilon^t, y^{t-1}) = p(i_t | \epsilon_t) = \symbf{1}(i(\epsilon_t) = i_t)
\end{eqnarray*}
