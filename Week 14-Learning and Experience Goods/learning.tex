...
\title{Learning Models and Experience Goods}
\author{C.Conlon}
\institute{Grad IO }
\date{\today}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[height=0.9\textheight]{resources/acker2.pdf}
\label{default}
\end{center}
\end{figure}
\end{frame}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[height=0.9\textheight]{resources/acker3.pdf}
\label{default}
\end{center}
\end{figure}
\end{frame}
...
\begin{eqnarray*}
\ell_i(\theta) &=& \Pr[c_{i1},\ldots,c_{iT_i} \mid W_i^t, Z_i^t, p_i^t; \theta] \\
&=& \int \Pr[c_{i1},\ldots,c_{iT_i} \mid W_i^t, Z_i^t, p_i^t; a_i;  \theta] f(d \alpha_i \mid \theta)\\
&=& \int \prod_{t=1}^{T_i} \Pr[c_{it}\mid  X_{it}(c_{i}^{t-1}), Z_{it}, p_{it}; a_i;  \theta] f(d \alpha_i \mid \theta)
\end{eqnarray*}
\begin{itemize}
...
\begin{align*}
\E[U_{ij} \mid I_i(t) ] = a_j -w_P P_j + w_E \sum_{s=0}^t D_{1ijs}  + w_{Ad} \sum_{s=t_0}^t D_{2ijs}
\end{align*}
\begin{itemize}
...
\item $\E[U_{ij}(t) \mid I_i(t)]$ is current period expected utility conditional on information set $I_i(t)$.
\end{itemize}
Consumers maximize a discounted stream of expected utilities producing the Bellman:
\begin{eqnarray*}
V_{ij}(I_i(t),t) &=& \E[U_{ij}(t) \mid I_i(t)] + \beta \cdot \E[V(I(t+1),t+1) \mid I(t)] \\
...
\E[U_{ijt} \mid I_i(t)] &=& -w_j P_{ijt} + w_A \E[A_{E_{ijt}} \mid I(t)] - w_A r \E [A_{E_{ijt}} \mid I_i(t)]^2\\
&&-w_A r \E[A_{E_{ijt}} - \E[A_{E_{ijt}}^2 \mid I_i(t)]]^2 + e_{ijt}
\end{eqnarray*}
...
Consumers update:
\begin{eqnarray*}
\E[A_{E_{ij,t+1}} \mid I_i(t)] &=&  \E[A_{E_{ijt}} \mid I_i(t-1)] \\
 &-& D_{1ijt} \beta_{1ij}(t) [A_{E_{ijt}} - \E[A_{E_{ijt}} \mid I_i(t-1)] ] \\
 &+&D_{2ijt} \beta_{2ij}(t) [S_{E_{ijt}} - \E[S_{E_{ijt}} \mid I_i(t-1)] ] \\
\end{eqnarray*}
\end{frame}
...
v_{ij} &= E[A_{ij} \mid I_{ij}(t)] - A_j
\end{align*} 
\item And
\begin{align*}
A_{j} &= \E[A_j \mid I_{ij}(t)] + v_{ij}(t)\\
...
\begin{align*}
\E[U_{ij} \mid I_i(t)] &= w_A A_j - w_A r A_j^2 -w_A r \sigma_{\delta}^2 -w_P P_{ij} \\
& -w_A r \sigma_{vij}^2(t) - w_A r v_{ij}(t)^2  - w_A v_{ij}(t) - 2w_A r A_j v_{ij} (t) \\ &+ e_{ijt}\\
\E[V_{ij} \mid I_i(t)] &= \E[U_{ij} \mid I_i(t)] + \beta \E[V_{ij} \mid I_i(t+1) \mid d_{ijt} =1, I_i(t)]
\end{align*}
\end{frame}
...
\begin{align*}
P^s_i(I(t),t) &= \int \frac{\exp [\E [U_{ij} \mid I_i(t)] ] } {\sum_k \exp [E [U_{ik} \mid I_i(t)] ] } f(v) dv\\
P^d_i(I(t),t) &= \int \frac{\exp [\E [V_{ij} \mid I_i(t)] ] } {\sum_k \exp [\E [V_{ik} \mid I_i(t)] ] } f(v) dv
\end{align*}
\begin{itemize}
...
\begin{eqnarray*}
h_{jt}(h_{j,t-1},y_{jnt}) = \frac{ \left(\frac{h_{j,t-1}}{1-h_{j,t-1}} \right) + d_{jnt} \cdot y_{jnt}}  {1+ \left(\frac{h_{j,t-1}}{1-h_{j,t-1}} \right) + d_{jnt} \cdot y_{jnt}}  
\end{eqnarray*}
...
&&+ \beta \cdot \E[(1-h_{jt}(h_{j,t-1},y_{jnt})\cdot  \E[W(S_{t+1}) \mid x_{jnt},y_{jnt},d_n=1] \mid S_t]\\
&=& \log [ \sum_n \exp[ \tilde{EU}(s) + \beta \E[(1-h(s'))W(s') \mid d_n=1] \mid S_t] \\
&=& \max_n \{W_n(S_t)\}
\end{eqnarray*}
\end{frame}
...
\begin{eqnarray*}
\E[W(s \mid d_n =1,s)] = \frac{1}{M} \sum_m (1-h(s^m)) W^0(s^m)
\end{eqnarray*}
...
\begin{eqnarray*}
&&\sum_{k=1}^K p_k \E_{\overline{x}_{jn T_{j}},k \mid h_{0,j,k}} \left[ \prod_{t=1}^{T_j-1} \left((1-h_{jt,k}) \prod_n \lambda_{jnt,k}^{d_{jnt}} \right) \right] \cdot h_{jT_j,k} \prod_n  \lambda_{jnt,k}^{d_{jnt}}\\
&&\sum_{k=1}^K p_k \E_{\overline{x}_{jn T_{j}},k \mid h_{0,j,k}} \left[ \prod_{t=1}^{T_j-1} \left((1-h_{jt,k}) \prod_n \lambda_{jnt,k}^{d_{jnt}} \right) \right] \cdot \prod_n  \lambda_{jnt,k}^{d_{jnt}}\\
\end{eqnarray*}
...
\[
\max_{j\in 1,...,J} \E(p_{i,j,T+1} \mid a_{0},b_{0},\widehat{Y}_{ij}) +
...
\begin{eqnarray*}
\text{Prob}_{j\in c,t} &=&\text{Prob}_{c,t}\text{(Prob}_{j,t}\mid \text{1\{c
chosen\})} \\
...
p_{j}\mid X_{ij} &\backsim &Beta(a_{0},b_{0}) \\
...
\begin{eqnarray*}
V(p_{j} \mid X_{ij})&=&\frac{\mu(1-\mu)}{1+\phi}
\end{eqnarray*}
