...
\end{eqnarray*}
The idea is to allow for different locations $l$ to obtain different utility from different products $j$. Assume (again) that the outside good exists alone in its own nest. We can write the share of product $j$ nationwide as $\sigma_j = \sum_{l=1}^L \omega_l \sigma_{jl}$ as the share of people $\omega_l$ who live in location $l$ and the choice probability of $j$ within $l$: $\sigma_{jl}$. We can decompose $\sigma_{jl} =  \sigma_{lc} \cdot \sigma_{jl|c}$ as the ``product of two logits''.\\
...
\noindent 1) [Very Easy] Write the within group share of $\sigma_{jl | c}$ assuming that $\eta_{jl}$ is observed for a single market $l$. It will be a function of the usual numerator and the denominator function $D_{lc}$.\\

\noindent 2) [Easy]  Write $\sigma_{lc}$ and $\sigma_{l0}$ as a function of only $D_{lc}$ for a single market $l$.  (ie: nothing product specific).\\

\noindent 3) Now write $D_{lc}$ as a function of only $\sigma_{lc}$ and $\sigma_{l0}$ and the unknown parameters for a single market $l$. (ie: nothing product specific).\\

\noindent 4) Now write $\sigma_j = \sum \omega_l \cdot \sigma_{lg} \cdot \sigma_{lj|g}$ as an expression which depends only on: $\sigma_j, \sigma_{l0},\sigma_{lc},\eta_{jl},\lambda$ and features of $j$ but not on features of other products.\\
...
\noindent Some trickery to avoid complicated proofs. Suppose that there are a lot of products per category so that you observe $\sigma_{lc}, \sigma_{l0}$ without any error. Moreover assume that any single product has a negligible effect on $\sigma_{lc}$. The idea is that you can treat the observed values of $\sigma_{lc}, \sigma_{l0}$ and thus $D_{lc}$ as data that are unaffected by your parameters.\\
...
\noindent 2) Write the individual surplus function $S_i = E[\max_j u_{ij}]$ and the total welfare function $W(t)$ (for all consumers and the representative firm) where $\pi(p) = p \cdot q- c(q)$ (where $q$ is the stacked vector of $q_{j}$ and $q_j = \sum_i n_i \sigma_{ij}$, the sum of individual demands).\\


\noindent 3) Use the envelope theorem to show that $\frac{d W(t)}{d\, t} = t \frac{d\, q_1(t)}{d t_1}$. for a tax $t_1$ on good $1$ that is returned lump sum to consumers.\\
